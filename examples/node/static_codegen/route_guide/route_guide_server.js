...
const checkFeature = (point) => {
  ...
  for (const i = 0; i < feature_list.length; i++) {
 ...
 * @param {(Error, feature) => } callback Response callback
 */
 ...
 * @param {(Error, routeSummary) => } callback The callback to pass the
 *     response to
 */
const recordRoute = (call, callback) => {
  const point_count = 0;
...
if (require.main === module) {
  ...
  fs.readFile(path.resolve(argv.db_path), (err, data) => {
    ...
    feature_list = _.map(JSON.parse(data), (value) => {
